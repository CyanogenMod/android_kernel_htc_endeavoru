...
{
	if (!blocked)
		endeavortd_config_bt_on();
	else
		endeavortd_config_bt_off();

	return 0;
}

static const struct rfkill_ops endeavortd_rfkill_ops = {
...
static struct platform_driver endeavortd_rfkill_driver = {
	.probe = endeavortd_rfkill_probe,
	.remove = endeavortd_rfkill_remove,
	.driver = {
...
static int __init endeavortd_rfkill_init(void)
{
	if(!machine_is_endeavortd())
		return 0;
	return platform_driver_register(&endeavortd_rfkill_driver);
}

static void __exit endeavortd_rfkill_exit(void)
{
	platform_driver_unregister(&endeavortd_rfkill_driver);
}

module_init(endeavortd_rfkill_init);
module_exit(endeavortd_rfkill_exit);
MODULE_DESCRIPTION("endeavortd rfkill");
