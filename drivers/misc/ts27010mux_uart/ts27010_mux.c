...
/* +++ Stanley_Chang@Raw-IP */
#ifdef TS27010_NET
struct ts0710_con ts0710_connection;
#else
...
static int ts27010_close_channel(u8 dlci)
{
	...
	while (try--) {
		...
		mux_print(MSG_LIGHT, "Modem alive=%d, force close=%d!\n", mdm_alive, g_mux_force_close);
		if(mdm_alive == 0) break;	//force close when modem crash
		else if(g_mux_force_close) break;	//force close for RIL
	...
	if (dlci != 0) {
		...
		mutex_unlock(&ts0710->dlci[0].lock);
		mux_print(MSG_DEBUG, "Dec control ref %d, dlci=%d\n",
			ts0710->dlci[0].clients, dlci);
	}
...
void ts27010_mux_uart_line_close(int line)
{
	...
	if (closeCTRL) {
		...
		g_mux_force_close = 0;	//reset  force close
	}
	FUNC_EXIT();
}
...
int ts27010_mux_uart_line_ioctl(unsigned int cmd, unsigned long arg, int line)
{
	...
	/* +++ Stanley_Chang@Raw-IP */
#ifdef TS27010_NET
	//struct dlci_struct *dlci = tty->driver_data;
	...
	case TS0710MUX_IO_SET_DEBUG_LEVEL:
		if ((arg >= MSG_MSGDUMP) && (arg <= MSG_NONE)) {
			g_mux_uart_print_level = arg;
			pr_info("MUX_UART: new debug level = %d\n",
				g_mux_uart_print_level);
			return 0;
		} else {
			pr_err("MUX_UART: ignore invalid debug level = %d\n",
				arg);
		}
	...
	case TS27010IOC_ENABLE_NET:
		mux_print(MSG_DEBUG, "tty line is  = %d\n", line);
		mux_print(MSG_DEBUG, "dlci is  = %d\n", dlci);
		...
		if (copy_to_user((void __user *)arg, &nc, sizeof(nc)))
			return -EFAULT;
		mux_print(MSG_INFO, "net idex: %d\n", index);
		return index;
	case TS27010IOC_DISABLE_NET:
	...
	case TS0710MUX_IO_FORCE_CLOSE:
		...
		return 0;
	default:
		mux_print(MSG_ERROR, "Unknown ioctl");
		break;
	}
	FUNC_EXIT();
...
static int __init mux_init(void)
{
	...
	g_mux_uart_print_level = MSG_NONE;
#ifdef DUMP_FRAME
